...
public class ProjXShowinfo extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.projxshowinfo);// 设置布局文件
		...
		lvinfo.setOnItemClickListener(new OnItemClickListener()// 为ListView添加项单击事件
		{
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (strType == "btnoutinfo" | strType == "btnininfo") {// 判断如果是支出或者收入信息
					intent = new Intent(ProjXShowinfo.this, ProjXInfoManage.class);// 使用InfoManage窗口初始化Intent对象
					intent.putExtra(FLAG, new String[] { strid, strType });// 设置要传递的数据
				} else if (strType == "btnflaginfo") {// 判断如果是便签信息
					intent = new Intent(ProjXShowinfo.this, ProjXFlagManage.class);// 使用FlagManage窗口初始化Intent对象
	...
	private void ShowInfo(int intType) {// 用来根据传入的管理类型，显示相应的信息
		...
		case R.id.btnoutinfo:// 如果是btnoutinfo按钮
			...
			OutaccountDAO outaccountinfo = new OutaccountDAO(ProjXShowinfo.this);// 创建OutaccountDAO对象
		...
		case R.id.btnininfo:// 如果是btnininfo按钮
			...
			InaccountDAO inaccountinfo = new InaccountDAO(ProjXShowinfo.this);// 创建InaccountDAO对象
		...
		case R.id.btnflaginfo:// 如果是btnflaginfo按钮
			...
			FlagDAO flaginfo = new FlagDAO(ProjXShowinfo.this);// 创建FlagDAO对象
